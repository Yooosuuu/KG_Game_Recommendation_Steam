...
- Final scoring: the embedding score is first normalized, then fused with the metadata score using `df['score'] = (alpha * df['kg_score_norm']) + ((1 - alpha) * df['meta_score'])`. So the embedding is not the final score by itself; it provides the learned ranking signal that is combined with Neo4j-based heuristics. (alpha used in the code is 0.75, giving more weight to the embedding score but still considering metadata).
